...
Use Mode A for paper explanation, structured reading, contribution/method/result extraction, or a reviewed digest without slides. Mode A may produce a source-bound `reading-view.json` and its `paper-analysis.md` projection before approval. Stop the presentation path after preparing CKPT-1; do not confuse that gate with a prohibition on an already-authorized Paper-Tutor explanation.
...
For a reader-facing Mode A analysis, Codex may also write `reading-view.json` in the project. It is a derived teaching projection with its own schema and source bindings. The runtime validates it before rendering `paper-analysis.md`; it does not become checkpoint evidence, a new factual source, or a Mode B input. A stale or malformed view fails closed. Projects without this file remain readable through the legacy projection.
