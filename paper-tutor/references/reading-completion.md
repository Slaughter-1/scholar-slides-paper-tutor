...
When a matching Mode A `reading-view.json` or seven-part analysis exists, use it
to seed the Reading Card's mainline, mechanism order, decisive evidence, and
term choices. This is a navigation aid; recheck factual claims against the
selected evidence source and do not create a parallel full analysis. Asset
completion means the reusable materials were produced; it does not claim that
the reader has personally mastered the paper.
...
For a Benchmark-paper analysis, use the Benchmark Card as the projection source: `Motivation → Problem`; `Task + Input / Output + Dataset + Environment → Setting`; `Baseline → Baseline`; the benchmark construction and evaluation flow across `Dataset → Task → Environment/Agent → Metric → Result` supplies `Method`; `Metric` supplies any non-trainable evaluation objective; `Dataset + Models + Agent + Metric → Evaluation`; and `Result`, `Failure`, `Scaling`, `Validity`, and `Research Gap` supply their corresponding result, failure, and research-analysis content. Do not invent a trainable objective for a benchmark that only defines evaluation criteria.
