...
In Integrated Mode, a matching `reading-view.json` or `paper-analysis.md` is a
navigation and teaching projection. It can supply the order of explanation and
candidate terminology, but it never outranks the evidence class that supports a
Paper Fact. Validate its paper identity and source bindings before using it. If
the view is stale or malformed, report the failure and use only a separately
verified upstream artifact; do not silently treat a prose projection as evidence.
...
Use one compact reader-facing status line near the title, then place the full
machine-readable status block at the end of a complete document. The compact
line keeps the first screen readable; the full block remains mandatory for
traceability. `Analysis source` identifies the analysis pathway. `Evidence
source` identifies the highest-priority matching factual evidence class from
the precedence list below. Never put an input-discovery channel such as
`explicitly supplied artifact or project directory` or `matching verified
context already in the conversation` in either source field. In Integrated
Mode, state the `verification_status` supported by the artifacts.
...
placeholder. Immediately after the full mode block, render exactly one
plain-text depth line: `Depth: quick`, `Depth: deep`, or `Depth: research`. Do not
wrap the depth value in backticks. The compact line may use the Chinese form
`来源：...；CKPT-1 ...；讲解深度：...` and does not replace the full block.
...
A pending CKPT-1 status is a real verification state, not missing input. It must
be disclosed, but it does not block an explanation already authorized by the
user. It still blocks presentation generation and any operation that requires a
confirmed reviewed view.
