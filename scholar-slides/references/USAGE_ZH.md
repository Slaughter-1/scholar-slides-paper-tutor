...
Mode A 用于论文理解，不制作 PPT。默认先形成一页可扫读的阅读主线，再展开机制、证据和术语：

```text
论文 -> ingest/build -> extractive digest -> reading-view（可选） -> CKPT-1 candidate -> 人工明确批准
```

`reading-view.json` 由 Codex 根据来源证据编写，包含 30 秒主线、输入→输出机制、决定性证据、论证链和真正必要的术语解释；runtime 只校验来源绑定并生成 `paper-analysis.md`，不会自动补事实。旧项目没有该文件时仍可读取旧版分析。CKPT-1 前的内容是候选理解；用户明确批准后，标题、贡献、方法、实验结果和证据绑定才成为 `confirmed semantic view`。待确认状态不会阻止用户已经要求的 Paper-Tutor 讲解，但不能进入 PPT 生成。
