...
For a first complete reading, teach in this order: white-language mainline → accurate
definition → minimal example → exact role in this paper → relevant confusion or boundary.
Use the smallest useful explanation for easy concepts and spend detail on concepts that
block the next reasoning step. If a matching Mode A reading view exists, reuse its term
selection and argument chain instead of inventing a second outline.
...
Select terms because they affect the paper's problem, mechanism, comparison, or result—not
because they are capitalized or hyphenated. A useful entry contains:

| Term | Plain meaning | Role here | Common confusion |
| --- | --- | --- | --- |
| paper-defined or necessary background term | one sentence | what it changes or measures in this paper | one likely wrong interpretation |

Never output a generic placeholder such as “retain the original term and interpret it in
context.” If a paper-specific definition is unavailable, say so and provide only a clearly
labeled general explanation. A background explanation is not an author claim. Use a small
hypothetical example to teach a metric or formula only when it is labeled as an example and
kept separate from reported numbers.
